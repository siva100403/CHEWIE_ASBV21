	...
	while(1)
	{
		...
		case LIDMOD_STATE_IDLE:
			...
			case DG_MODULE_START:
				//Read Limit switch
				if(getLidSwicthStatus()== LIDCLOSE)
				{
			...
			case DG_TIMER_EXPIRY:
				//Timer event should not occur in IDLE state. Stope timer
				dgtimerStop(LID_MOD);
				lidMotorStop();
				break;
			default:
		...
		case LIDMOD_STATE_OPENING:
			...
			case DG_TIMER_EXPIRY:
				...
				lidMotorStop();
				lidModuleState = LIDMOD_STATE_OPEN_WAIT;
				dgtimerStart(LID_MOD, LID_OPEN_WAIT/portTICK_PERIOD_MS);
				break;
			default:
		...
		case LIDMOD_STATE_OPEN:
			...
			break;
		case LIDMOD_STATE_OPEN_DELAY:
			printf("lidModule.c:lidModule_task():Rcvd Event:%d in state LIDMOD_STATE_OPEN_DELAY)\r\n",rcvMsg.command);
			switch(rcvMsg.command)
			{
			case DG_MODULE_START:
				//Ignore. Already active
			...
			case LIDSWITCH_EVENT:
				...
				break;
			case DG_TIMER_EXPIRY:
				//Timer expired. Now we can close the lid
				lidOpen();
				lidModuleState = LIDMOD_STATE_OPENING;
				dgtimerStart(LID_MOD, LIDOPENING_DURATION/portTICK_PERIOD_MS);
				break;
			default:
				printf("lidModule.c:lidModule_task():Invalid Event:%d in state LIDMOD_STATE_OPEN_DELAY - Ignored)\r\n",rcvMsg.command);
				break;
			}
			break;
		case LIDMOD_STATE_CLOSE_WAIT:
			printf("lidModule.c:lidModule_task():Rcvd Event:%d in state LIDMOD_STATE_CLOSE_WAIT)\r\n",rcvMsg.command);
			switch(rcvMsg.command)
			{
			case DG_MODULE_START:
				//Ignore. Already active
			...
			case DG_TIMER_EXPIRY:
				...
				break;
			default:
				printf("lidModule.c:lidModule_task():Invalid Event:%d in state LIDMOD_STATE_CLOSE_WAIT - Ignored)\r\n",rcvMsg.command);
				break;
			}
			break;
		case LIDMOD_STATE_OPEN_WAIT:
			printf("lidModule.c:lidModule_task():Rcvd Event:%d in state LIDMOD_STATE_OPEN_WAIT)\r\n",rcvMsg.command);
			switch(rcvMsg.command)
			{
			case DG_MODULE_START:
				//Ignore. Already active
			...
			case PROXIMITY_EVENT:
				...
				break;
			case LIDSWITCH_EVENT:
				//Ignore. No Limit switch for OPEN
				//lidMotorStop();  //In case the motor direction is reversed for OPEN
				TODO
				break;
			case DG_TIMER_EXPIRY:
				//Lid open duration reached. Stop lid motor
				lidMotorStop();
				lidModuleState = LIDMOD_STATE_OPEN;
				if(capProximityState == CAP_PROXIMITY_EVENT_REMOVED)
				{
					//Start timer to close the Lid
					dgtimerStart(LID_MOD, CONV_SEC_TO_TICKS(NONPROXIMITY_LIDCLOSE_TIMEOUT));
					capProximityState = CAP_PROXIMITY_EVENT_REMOVED;
				}
				else
				...
				break;
			default:
				printf("lidModule.c:lidModule_task():Invalid Event:%d in state LIDMOD_STATE_OPEN_WAIT - Ignored)\r\n",rcvMsg.command);
				break;
			}
			break;
...
int initLidModule(void)
{
	...
	result = xTaskCreate(lidModule_task, "lidModule_task", configMINIMAL_STACK_SIZE + 100, NULL, task_PRIORITY, &lidModuleTaskHandle);
    if ( result !=    pdPASS)
    {
